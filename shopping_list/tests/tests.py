...
@pytest.mark.django_db
def test_shopping_list_retrieved_by_id(create_user, create_authenticated_client, create_shopping_list):
    user = create_user()
    client = create_authenticated_client(user)
...
def test_shopping_list_includes_only_corresponding_items(create_user, create_authenticated_client, create_shopping_list):
    ...
    client = create_authenticated_client(user)
    shopping_list = create_shopping_list(user, 'Groceries')
    another_shopping_list = create_shopping_list(user, 'Books')
...
def test_shopping_list_name_is_changed(create_user, create_authenticated_client, create_shopping_list):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert response.data['name'] == 'Foods'
...
def test_add_members_list_member(create_user, create_authenticated_client, create_shopping_list):
    ...
    third_member = User.objects.create(username='thir_member', password='whocares')
